...
class FileEncrypto:
    ...
    def encrypt_directory(self, directory_path):
        ...
        with open(os.path.join(directory_path, 'key'), 'w') as key_file:
...
class FileDecryptor:
    ...
    def decrypt_directory(self, directory_path):
        ...
        for file_name, cipher in self.ciphers.items():
            encrypted_file_path = os.path.join(directory_path, file_name + '.encrypted')
            try:
...
if __name__ == "__main__":
    ...
    keys = []
    with open(os.path.join(directory_path, 'key'), 'r') as key_file:
